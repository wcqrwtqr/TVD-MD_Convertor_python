...
#what is happening here and what is the different 
...
with open(list_of_MD_file_path,'r') as f:
    ...
    MD_in = []
    for row in data_in:
        if row: MD_in.append(float(row[0])) 
...
def calTVD (MDi):
    ...
    if float(MDi) < float(MD[0]):
        print('The values of MD should be between {0}m and {1}m the selected {2}m value is not valid'.format(MD[0], MD[-1], MDi)) 
    elif float(MDi) > float(MD[-1]):
        print('The values of MD should be between {0}m and {1}m the selected {2}m value is too High'.format(MD[0], MD[-1], MDi))
    else:
        #find the index to start calculation
        indx = [i for i in range(0,len(MD)) if float(MD[i])<= MDi and
                float(MD[i+1])>= MDi]
        indint = indx[0]
        #Calculation bolck
